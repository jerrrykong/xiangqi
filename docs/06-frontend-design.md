...
| 自建 UI 组件 | — | AppConfirm/AppToast 替代 Element Plus |
| 原生 WebSocket | — | 全局唯一长连接，承载所有业务通信 |

> **变更说明**: 移除 Element Plus 依赖，所有 UI 交互（确认框、Toast 提示）改为自建组件，
> 通过 `showConfirm()` / `showToast()` 编程式调用。按钮、表单、弹窗均为纯 CSS 实现。

## 2. 目录结构

```
cmd/web/src/
├── main.ts                     # 应用入口 (Pinia + Router + styles/main.css)
├── App.vue                     # 根组件 (RouterView + WS处理器注册 + 断线检测)
├── auto-imports.d.ts           # 自动生成 — 全局类型声明
├── components.d.ts             # 自动生成 — 组件类型声明
├── components/                 # 组件 (3 个子目录, 9 个组件)
│   ├── chess/
│   │   ├── ChessBoard.vue      # 棋盘组件 (SVG棋盘背景 + SVG棋子图片 + JS动态缩放)
│   │   ├── ChessPiece.vue      # 单枚棋子组件 (SVG图片 + 选中高亮 + 走棋动画)
│   │   └── MoveList.vue        # 着法记录列表 (红黑配对, 高亮当前步, emit jump)
│   ├── common/
│   │   ├── AppConfirm.vue      # 确认对话框 (替代 ElMessageBox, warning/danger/info)
│   │   ├── AppToast.vue        # 轻量提示通知 (替代 ElMessage, 自动消失)
│   │   └── ui.ts               # 编程式调用 (showToast/showConfirm, createApp+h挂载)
│   └── game/
│   │   ├── GameControls.vue    # 对局操作按钮组 (求和/认输/退出/再来一局/开始)
│   │   ├── GameTimer.vue       # 棋钟倒计时 (MM:SS, 低于阈值闪烁警告)
│   │   ├── PlaybackControls.vue# 复盘播放控制条 (prev/toggle/next/speed/seek)
│   │   └── PlayerInfo.vue      # 玩家信息 (头像SVG+名称+段位+计时器)
├── pages/                      # 页面组件 (9 个)
│   ├── Splash.vue              # 启动页 (自动重连/凭证认证/登录入口)
│   ├── Login.vue               # 登录页 (木质风格卡片布局)
│   ├── Register.vue            # 注册页 (username/password/confirm/nickname)
│   ├── Lobby.vue               # 游戏大厅 (SVG图标UI/匹配/创建房间/排行榜/历史)
│   ├── RoomList.vue            # 房间列表页 (分页20条/页)
│   ├── Game.vue                # 对局页面 (新布局: topbar+center+sidebar)
│   ├── GameRoom.vue            # 房间等待页 (轮询1s间隔)
│   ├── Review.vue              # 棋局复盘页 (框架阶段, 播放控制+弹出面板)
│   └── Settings.vue            # 系统设置页 (音效/棋盘主题/账号/关于)
├── router/
│   └── index.ts                # 路由定义 + 守卫 (8条路由, 全懒加载, WS鉴权守卫)
├── stores/                     # Pinia 状态管理 (4 个)
│   ├── auth.ts                 # 认证 Store (凭证/登录/重连/状态机, 18KB)
│   ├── game.ts                 # 游戏 Store (棋盘/走棋/将军检测/记谱/音效/动画, 42KB)
│   ├── match.ts                # 匹配 Store (排队/匹配结果, 2KB)
│   └── room.ts                 # 房间 Store (房间列表/当前房间/创建/加入/离开, 11KB)
├── styles/                     # CSS 模块化样式系统 (15 个文件)
│   ├── main.css                # 主入口 (按顺序导入所有模块)
│   ├── reset.css               # CSS 重置/normalize
│   ├── variables.css           # 设计变量系统 (中国传统木质暖色系, 7KB)
│   ├── global.css              # 全局基础样式 + 头像组件样式
│   ├── buttons.css             # 按钮样式 (btn-primary/secondary/text/danger/block)
│   ├── forms.css               # 表单样式 + Toggle开关组件
│   ├── popups.css              # 弹出面板/遮罩/overlay样式
│   ├── board.css               # 棋盘样式
│   ├── pieces.css              # 棋子样式
│   ├── login.css               # 登录页样式
│   ├── lobby.css               # 大厅页样式
...
│   ├── review.css              # 复盘页样式
│   ├── settings.css            # 设置页样式
│   └── _compat.css             # 兼容性样式 (旧变量映射)
├── types/
│   ├── api.ts                  # Difficulty枚举 + DifficultyLabels
...
├── assets/
│   └── svg/
│       ├── board.svg            # 棋盘SVG (格线+九宫+楚河汉界+星位四方角)
│       ├── pieces/              # 棋子SVG图片 (红7+黑7, 共14个)
│       │   ├── red-shuai.svg    # 红帅
│       │   ├── red-shi.svg      # 红仕
│       │   ├── red-xiang.svg    # 红相
│       │   ├── red-ma.svg       # 红马 (傌)
...
│       └── ui/                  # UI SVG图标 (24个)
│           ├── logo.svg         # 棋型Logo
│           ├── text-logo.svg    # 文字Logo "楚汉争锋"
│           ├── btn-bg-normal.svg# 按钮背景
│           ├── icon-*.svg       # 20个功能图标 (sword/ai/trophy/clock/settings/
│           │                    #   plus/user/exit/close/back/flag/refresh/play/
│           │                    #   pause/check/star/undo/fullscreen/import/
│           │                    #   sound-on/sound-off/next/prev)
└── sounds/                      # 音效文件
    ├── sfx_*.wav                # 音效 (pickup/putdown/capture/check/win/lose/draw)
    ├── voice_start*.wav         # 语音: 开始
    ├── voice_move_*.wav         # 语音: 走棋 (chariot/pawn)
    ├── voice_ping_cannon*.wav   # 语音: 平炮
    ├── voice_eat_*.wav          # 语音: 吃子
    ├── voice_check*.wav         # 语音: 将军
    ├── voice_red_win*.wav       # 语音: 红方胜
    ├── voice_black_win*.wav     # 语音: 黑方胜
    └── voice_draw*.wav          # 语音: 和棋
```
...
| `showResultDialog` | `boolean` | 显示结果弹窗 |
| `winner` | `number \| null` | 胜方 (yourColor值 / -1=和棋) |
...
### 7.1 ChessBoard.vue (`components/chess/`)

基于 SVG 棋盘背景 + SVG 棋子图片的中国象棋棋盘组件。

**渲染方式**：
- 棋盘背景：`<img src="board.svg">`，JS 动态计算缩放尺寸（不使用 CSS transform）
- 棋子渲染：`ChessPiece.vue` 组件，根据 `board[][]` 数据映射到对应 SVG 图片文件名

**棋盘 SVG (`public/assets/svg/board.svg`)**：
- 格线：9列×10行，间距 60px，起始于 (30, 30)
- 九宫对角线：上方 (col 3-5, row 0-2)，下方 (col 3-5, row 7-9)
- 楚河汉界：row 4-5 之间，仅绘制左右两列纵线
- 星位标记：炮位(2,1)/(2,7)/(6,1)/(6,7) + 兵/卒位，使用传统四方角 L 形标记

**棋子 SVG (`public/assets/svg/pieces/*.svg`)**：
- 红方7个 + 黑方7个，圆形棋子带书法字体
- 文件名映射：`color-piece.svg`，如 `red-shuai.svg`、`black-jiang.svg`
...
- 走棋动画: CSS `piece-slide-in`，通过 `animDx/animDy` 偏移量计算
- 响应式缩放: JS 动态计算棋盘尺寸适配容器宽度

### 7.2 ChessPiece.vue (`components/chess/`)

单枚棋子渲染组件。

**Props**: `id`, `col`, `row`, `type` (棋子编码), `side` (红/黑), `selected`, `animDx`, `animDy`, `boardScale`

**渲染**: 根据 `type` 映射到 SVG 文件名 (`pieceSvgMap`)，支持选中高亮和走棋动画偏移。

### 7.3 MoveList.vue (`components/chess/`)

着法记录列表组件。红黑配对显示（每两步一行），支持 `currentMove` 高亮当前步，emit `jump` 事件。

### 7.4 GameControls.vue (`components/game/`)

对局操作按钮组，使用 SVG 图标。

**Props**: `phase` (playing/finished/ready), `canRematch`, `iWantRematch`, `opponentWantsRematch`, `isGameOver`
...
| 认输 | `icon-flag.svg` |
| 退出 | `icon-exit.svg` |
| 再来一局 | `icon-refresh.svg` |
| 返回大厅 | `icon-back.svg` |
| 开始 | `icon-play.svg` |

### 7.5 PlayerInfo.vue (`components/game/`)

玩家信息组件。头像使用 `icon-user.svg` 半透明底图 + 文字层叠，支持回合状态高亮。

**Props**: `side`, `name`, `level`, `time`, `isTurn`, `showTimer`

### 7.6 GameTimer.vue (`components/game/`)

棋钟倒计时组件。`MM:SS` 格式，低于 `warnThreshold`(默认30秒)时闪烁警告。

### 7.7 PlaybackControls.vue (`components/game/`)

复盘播放控制条。支持 0.5x/1x/2x/4x 播放速度，prev/toggle/next/speed/seek 事件。

### 7.8 AppConfirm.vue (`components/common/`)

确认对话框，替代 ElMessageBox。Props: `message`, `title`, `confirmText`, `cancelText`, `type`(warning/danger/info)。返回 Promise<boolean>。

### 7.9 AppToast.vue (`components/common/`)

轻量提示通知，替代 ElMessage。Props: `message`, `type`(success/error/warning/info), `duration`。自动定时消失。

### 7.10 ui.ts (`components/common/`)

编程式调用工具。通过 `createApp + h()` 动态创建组件实例挂载到 body，导出 `showToast()` 和 `showConfirm()` 函数。
...
木质风格卡片布局，自定义表单。字段: `username` (4-32字符), `password` (≥8字符)。支持 redirect 参数。

### 8.3 Register.vue — 注册页

同登录页风格。字段: `username`, `nickname`(选填), `password`(含字母+数字), `confirmPassword`。

### 8.4 Lobby.vue — 大厅页

**功能模块**：
- 用户信息卡片 (SVG头像装饰 + 用户名 + 积分 + 对局数)
- Header链接: 排行榜(`icon-trophy`)、历史记录(`icon-clock`)、设置(`icon-settings`)
- 操作按钮: 新对局(`icon-sword`)、快速匹配(`icon-refresh`)、人机对弈(`icon-ai`)、房间列表(`icon-plus`)、系统设置(`icon-settings`)
- PvE难度选择弹窗 (全屏overlay, 1-5级难度)
- 排行榜/历史弹窗 (全屏overlay, 可滚动)
- 底部链接: 排行榜(`icon-trophy`)、历史记录(`icon-clock`)、退出登录(`icon-exit`)
- 等待对手卡片 (创建房间后显示)
- 断线重连提示
...
**新布局**：
- topbar: logo + 房间号 + 轮次指示 + 音效开关(`icon-sound-on/off`)
- center: 对手信息 → 棋盘 → 操作按钮 → 自己信息
- sidebar-desktop: 着法记录 (≥769px 显示)
- mobile: 底部切换栏(`icon-clock`) + bottom sheet 着法面板

**功能按钮** (GameControls组件):
- Playing: 求和(`icon-undo`)、认输(`icon-flag`)、退出(`icon-exit`)
- Ready: 开始(`icon-play`)、退出
- Finished: 再来一局(`icon-refresh`)、返回大厅(`icon-back`)

**结果弹窗**: SVG图标(胜→`icon-trophy`、和→文字"和"、负→`icon-flag`) + 胜负描述 + 积分变化

**求和弹窗**: `icon-undo` 标题 + 同意/拒绝

**浮层提示**: WS断连、AI思考中、等待对手加入、等待对方开始

**音效开关**: `icon-sound-on` / `icon-sound-off` 动态切换

### 8.7 GameRoom.vue — 房间等待页

玩家创建/加入房间后等待对手，轮询房间状态(1秒间隔)。

### 8.8 Review.vue — 复盘页 (框架阶段)

棋盘展示 + PlaybackControls播放控制 + 弹出面板(着法记录/对局信息)。当前用空棋盘占位。

### 8.9 Settings.vue — 设置页

三个区块：对局设置(音效开关、棋盘主题、走棋确认)、账号管理、关于信息。使用全局 `.toggle` 开关组件(定义在 `forms.css`)。退出登录按钮。
...
| `/review/:gameId` | Review | Review.vue | 是 | 棋局复盘 |
| `/settings` | Settings | Settings.vue | 是 | 系统设置 |
...
### 12.1 变量系统 (`variables.css`)

中国传统木质暖色系设计变量，7KB，涵盖：

**主色系**：
- `--color-gold: #D97706` / `--color-gold-light: #F59E0B` / `--color-gold-dark: #B45309`
- `--color-wood-50` ~ `--color-wood-900` (木质棕色系)
- `--color-piece-red: #c41e3a` / `--color-piece-black: #1a1a1a`
- `--color-board-line: #8b5a2b` / `--color-board-bg: #deb887`

**字体**：
- `--font-serif: 'Noto Serif SC', serif` (思源宋体，Google Fonts)
- `--font-sans: 'Noto Sans SC', sans-serif`
- `--font-mono: 'JetBrains Mono', monospace`

**间距/圆角/阴影/动画**: 全套 CSS 变量定义
...
| `main.css` | 主入口，按顺序导入所有模块 |
| `reset.css` | CSS 重置/normalize |
| `variables.css` | 设计变量系统 (7KB) |
| `global.css` | 全局基础样式 + 头像组件样式 |
| `buttons.css` | 按钮样式 (primary/secondary/text/danger/block) |
| `forms.css` | 表单样式 + Toggle开关组件 |
| `popups.css` | 弹出面板/遮罩/overlay样式 |
| `board.css` | 棋盘样式 |
| `pieces.css` | 棋子样式 |
...
| `review.css` | 复盘页样式 |
| `settings.css` | 设置页样式 |
| `_compat.css` | 兼容性样式 (旧变量名映射) |

### 12.3 Toggle 开关组件 (定义在 `forms.css`)
...
- `.toggle-track`: 44×24px 药丸形，灰色底→金色(gold)选中态
- `.toggle-track::after`: 20px 白色圆点，`translateX(20px)` 滑动

## 13. SVG 资源系统

### 13.1 UI 图标 (`public/assets/svg/ui/`)

24 个 SVG 文件，用于替代所有页面的 emoji：

| 类别 | 图标文件 | 用途 |
|------|---------|------|
| Logo | `logo.svg`, `text-logo.svg` | 品牌/标题 |
| 按钮 | `btn-bg-normal.svg` | 按钮背景纹理 |
| 对局 | `icon-sword.svg`, `icon-ai.svg`, `icon-flag.svg`, `icon-undo.svg` | 新对局/人机/认输/求和 |
| 导航 | `icon-back.svg`, `icon-exit.svg`, `icon-plus.svg`, `icon-refresh.svg` | 返回/退出/加入/刷新 |
| 信息 | `icon-trophy.svg`, `icon-clock.svg`, `icon-star.svg`, `icon-user.svg` | 排行/历史/星级/头像 |
| 媒体 | `icon-play.svg`, `icon-pause.svg`, `icon-next.svg`, `icon-prev.svg` | 播放控制 |
| 功能 | `icon-settings.svg`, `icon-close.svg`, `icon-check.svg`, `icon-fullscreen.svg`, `icon-import.svg` | 设置/关闭/校验/全屏/导入 |
| 音效 | `icon-sound-on.svg`, `icon-sound-off.svg` | 音效开关 |

**引用方式**: `<img :src="baseUrl + 'assets/svg/ui/icon-xxx.svg'">`

### 13.2 棋盘与棋子 (`public/assets/svg/`)

| 文件 | 说明 |
|------|------|
| `board.svg` | 棋盘SVG (540×600px): 格线+九宫斜线+楚河汉界+星位四方角 |
| `pieces/*.svg` | 14个棋子SVG: 红方7个(red-shuai/shi/xiang/ma/ju/pao/bing) + 黑方7个 |

**棋盘 SVG 坐标系统**：
- 列: col 0→x30, col 1→x90, ... col 8→x510 (间距60px)
- 行: row 0→y30, row 1→y90, ... row 9→y570 (间距60px)
- 九宫斜线: 上方(col 3-5, row 0-2), 下方(col 3-5, row 7-9)
- 星位四方角: L形角标(g=5, l=10), 左右边缘只显示半侧

## 14. 构建配置 (`vite.config.ts`)

```typescript
export default defineConfig({
  plugins: [vue(), AutoImport({ imports: ['vue','vue-router','pinia'] }), Components({ dts })],
  base: '/xiangqi/',
  resolve: { alias: { '@': './src' } },
  server: {
    port: 5173,
    proxy: {
      '/xiangqi/ws': { target: 'ws://localhost:8765', ws: true, rewrite: path → '/ws' }
    }
  }
})
```
...
- **proxy**: WS 代理，前端 `/xiangqi/ws` → 后端 `ws://localhost:8765/ws`
